...
void main() {
  ...
  float z1x = texture(DEPTHMAP, (p+1.)*.5 + vec2(PX_SIZE.x*.5, 0)).r;
  float z1y = texture(DEPTHMAP, (p+1.)*.5 + vec2(0, PX_SIZE.y*.5)).r;

  vec3 P0 = vec3(p, z0);
  vec3 P1x = vec3(p+vec2(PX_SIZE.x,0), z1x);
  vec3 P1y = vec3(p+vec2(0,PX_SIZE.y), z1y);
